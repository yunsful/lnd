...
func RegisterLightningJSONCallbacks(registry map[string]func(ctx context.Context,
	...
	registry["lnrpc.Lightning.SendChannelAnnouncement"] = func(ctx context.Context,
		conn *grpc.ClientConn, reqJSON string, callback func(string, error)) {

		req := &SendChannelAnnouncementRequest{}
		err := marshaler.Unmarshal([]byte(reqJSON), req)
		if err != nil {
			callback("", err)
			return
		...
		client := NewLightningClient(conn)
		resp, err := client.SendChannelAnnouncement(ctx, req)
		if err != nil {
			callback("", err)
			return
		}
